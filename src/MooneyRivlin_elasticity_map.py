...
def main(refData, network):
    ...
    def pde(x, f):
        ...
        Pxx = mu1 * Fxx + mu2 * (I1 * Fxx - Fxx * Cxx - Fxy * Cyx)
        Pxy = mu1 * Fxy + mu2 * (I1 * Fxy - Fxx * Cxy - Fxy * Cyy)
        Pyx = mu1 * Fyx + mu2 * (I1 * Fyx - Fyx * Cxx - Fyy * Cyx)
        Pyy = mu1 * Fyy + mu2 * (I1 * Fyy - Fyx * Cxy - Fyy * Cyy)
        ...
        if "1" in network or "2" in network:
            stress_xx = (Pxx - NPxx) / (mu1)
            stress_yy = (Pyy - NPyy) / (mu1)
            stress_xy = (Pxy - NPxy) / (mu1)
            stress_yx = (Pyx - NPyx) / (mu1)
    ...
    def output_transform(x, y):
        Nux, Nuy, Nmu1 = y[:, 0:1], y[:, 1:2], y[:, 2:3]
        ...
        return torch.concat(
            [Nux, Nuy, Nmu1, Fxx, Fyy, Fxy, Fyx, Exx, Eyy, Exy, Pxx, Pyy, Pxy, Pyx],
            axis=1,
        )
...
def plot_data(refData):
    ...
    pinn_coor, pinn_mu1_map = (
        PINN.item()["Coor"],
        PINN.item()["PINN output"][:, 2],
    )

    gt_mu1_map = griddata(gt_coor[:, 0:2], gt_mu1_map, (X, Y), method="cubic")
    pinn_mu1_map = griddata(pinn_coor, pinn_mu1_map.reshape(-1), (X, Y), method="cubic")

    print(
        "Young's modulus L2 relative error (%): ",
        np.linalg.norm(gt_mu1_map - pinn_mu1_map) / np.linalg.norm(gt_mu1_map) * 100,
    )
    ...
    plt.colorbar(
        ...
        ticks=[
            np.min(gt_mu1_map),
            (np.max(gt_mu1_map) + np.min(gt_mu1_map)) / 2,
            np.max(gt_mu1_map),
        ],
        format="%.2f",
    )
    ...
    plt.colorbar(
        ...
        ticks=[
            np.min(pinn_mu1_map),
            (np.max(pinn_mu1_map) + np.min(pinn_mu1_map)) / 2,
            np.max(pinn_mu1_map),
        ],
        format="%.2f",
    )
    ...
    ax = axes[2]
    abs_error = np.abs(gt_mu1_map - pinn_mu1_map)
    ...
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title(r"|$\mu_1$ - $\mu_1^*$|")

    plt.tight_layout()
    plt.savefig("compare_mu1.pdf")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data", type=str, default="../data/GRF_equi_disp0.4_MR.npy")
    ...
    if "MR" not in args.data:
        print("Reference data are not characterized by Mooney-Rivlin model.")
        exit()

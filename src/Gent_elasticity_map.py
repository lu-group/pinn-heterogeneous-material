...
def main(refData, network):
    ...
    def pde(x, f):
        ...
        if "1" in network or "2" in network:
            stress_xx = (Pxx - NPxx) / (mu)
            stress_yy = (Pyy - NPyy) / (mu)
            stress_xy = (Pxy - NPxy) / (mu)
            stress_yx = (Pyx - NPyx) / (mu)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default="../data/GRF_equi_disp0.4_Gent.npy")
    ...
    if "Gent" not in args.data:
        print("Reference data are not characterized by Gent model.")
        exit()
